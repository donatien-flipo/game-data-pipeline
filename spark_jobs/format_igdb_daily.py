...
from pyspark.sql.types import StructType, StructField, StringType, LongType, DoubleType, ArrayType, BooleanType, IntegerType

def create_spark_session():
    """Configuration Spark pour MinIO (S3A)."""
    minio_endpoint = os.getenv("MINIO_ENDPOINT", "http://minio:9000")
    minio_user = os.getenv("MINIO_ROOT_USER", "admin")
    minio_password = os.getenv("MINIO_ROOT_PASSWORD", "password123")

    spark = SparkSession.builder \
        .appName("IGDB Raw to Parquet") \
...
def transform_igdb(execution_date):
    spark = create_spark_session()
    
    input_path = f"s3a://data/raw/igdb/daily_games/{execution_date}/games.json"
    output_path = f"s3a://data/formatted/igdb/games/dt={execution_date}"
    ...
    print(f"Transformation terminée et écrite vers {output_path}")

if __name__ == "__main__":
    date_arg = sys.argv[1] if len(sys.argv) > 1 else None
    transform_igdb(date_arg)