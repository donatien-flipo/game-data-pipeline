...
from pyspark.sql.types import StructType, StructField, StringType, LongType, ArrayType, BooleanType, IntegerType, FloatType
...
def transform_rawg(execution_date):
    spark = create_spark_session()
    
    input_path = f"s3a://data/raw/rawg/daily_games/{execution_date}/games.json"
    output_path = f"s3a://data/formatted/rawg/games/dt={execution_date}"

    df = spark.read.schema(rawg_schema).json(input_path, multiLine=True)

    if df.rdd.isEmpty():
        print("Aucune donnée RAWG à traiter.")
        return
    ...
    print(f"RAWG transformé et écrit vers {output_path}")

if __name__ == "__main__":
    date_arg = sys.argv[1] if len(sys.argv) > 1 else None
    transform_rawg(date_arg)